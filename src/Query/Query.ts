...
class QueryProvider implements Query {
    ...
    public where(attributeName: string, operator: QueryOperator, ...values: any[]): Query {
    ...
    public whereAny(any: (or: (attributeName: string, operator: QueryOperator, ...values: any[]) => void) => void): Query {
        var conditions = [];
        any((attributeName: string, operator: QueryOperator, ...values: any[]) => {
            conditions.push({
                AttributeName: attributeName,
                Operator: operator,
                Values: this.flatten(values)

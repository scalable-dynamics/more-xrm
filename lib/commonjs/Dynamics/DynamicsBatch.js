...
class Batch {
    ...
    execute() {
        return __awaiter(this, void 0, void 0, function* () {
            const results = yield Batch.requestBatch(`/api/data/${Dynamics_1.WebApiVersion}/$batch`, this.Changes, this.headers);
            if (this.RelatedChanges.length > 0) {
                ...
                const related = yield Batch.requestBatch(`/api/data/${Dynamics_1.WebApiVersion}/$batch`, this.RelatedChanges, this.headers);
    ...
    static requestBatch(url, requests, headers) {
        ...
        return fetch(url, {
            method: 'POST',
            headers: Object.assign({ 'Content-Type': `multipart/mixed;boundary=batch_${batchId}` }, Dynamics_1.DynamicsHeaders, headers),
        ...
        for (let change of changes) {
            if (change.entitySetQuery) {
                ...
                if (change.entitySetName) {
                    requestBody.push(`GET ${encodeURI(`/api/data/${Dynamics_1.WebApiVersion}/${change.entitySetName}?${change.entitySetQuery}`)} HTTP/1.1`);
                }
                else {
                ...
                batchBody.push('');
                batchBody.push(`${change.entityId ? 'PATCH' : 'POST'} ${encodeURI(`/api/data/${Dynamics_1.WebApiVersion}/${change.entitySetName}(${Batch.trimId(change.entityId)})`)} HTTP/1.1`);
